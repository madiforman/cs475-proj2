...
int main(uint32 argc, uint32 *argv)
{
	...
	/*to highlight effectiveness of negative priorities (AGING must be TRUE)*/
	ready(create((void *)printpid, INITSTK, -1, "NEGATIVE 1", 2, 1, args1++), FALSE);
	ready(create((void *)printpid, INITSTK, -100, "MOST NEGATIVE", 2, 1, args1++), FALSE);

	/*to highlight to processes with large priorities competing*/
	ready(create((void *)printpid, INITSTK, 100, "MOST NEGATIVE", 2, 30, args1++), FALSE);
	ready(create((void *)printpid, INITSTK, 100, "MOST POSITIVE", 2, 12, args1++), FALSE);
